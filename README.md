...
*   **RAPIDAPI_KEY**: Buat akun di [RapidAPI](https://rapidapi.com/), cari API `instagram-scraper-stable-api` dan subscribe ke paket *Basic/Free*. Anda akan mendapatkan API Key yang bisa dipasang di file `.env.local`.
*   **YOUTUBE_API_KEY**: Buat project di [Google Cloud Console](https://console.cloud.google.com/), aktifkan **YouTube Data API v3**, dan buat *API Key* di bagian Credentials.
